document.addEventListener('DOMContentLoaded', () => {
    ...
    const map = L.map('map').setView([34.0522, -118.2437], 10); // Los Angeles, closer zoom
    ...
    // Add scale control (miles and feet)
    L.control.scale({
        imperial: true,
        metric: false,
    ...
    function loadDistrictRoutes(district) {
        ...
        script.onload = function () {
            ...
            // Add routes to map with thinner lines (0.5pt ≈ 1.5px)
            L.geoJSON(data, {
                style: function (feature) {
                    const speed = feature.properties.speed_mph || 0;
                    return {
                        color: getRouteColor(speed),
                        weight: 1.5, // 0.5pt thickness
                        opacity: 0.8
                    };
                },
            ...
            // Fit map to routes bounds with closer zoom
            if (data.features.length > 0) {
                const bounds = L.geoJSON(data).getBounds();
                map.fitBounds(bounds, {
                    padding: [30, 30],
                    maxZoom: 12 // Closer zoom limit
                });
            }
        };
    ...
    function updateWidgets(data) {
        ...
        if (data.healthcare_accessibility) {
            ...
            if (topRoutes.length > 0) {
                ...
                topRoutes.slice(0, 3).forEach(route => {
                    html += `<li><span class="label" style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:120px;" title="${route.route_name}">${route.route_name}</span> <span class="val">${Math.round(route.speed_mph)} mph</span></li>`;
                });
                html += '</ul>';
                document.getElementById('healthcare-extra').innerHTML = html;
            } else {
                document.getElementById('healthcare-extra').innerHTML = '<p style="font-size:0.8rem; color:var(--text-secondary);">No healthcare routes found.</p>';
            }
        }
        ...
        if (data.safe_passage) {
            ...
            if (data.safe_passage.avg_speed > 0) {
                ...
                document.getElementById('safe-passage-extra').innerHTML = '<p style="font-size:0.8rem; color:var(--text-secondary);">No school routes found.</p>';
            }
        }
        ...
        if (data.community_connection) {
            ...
            if (data.community_connection.avg_diff > 0) {
                ...
                document.getElementById('connection-extra').innerHTML = '<p style="font-size:0.8rem; color:var(--text-secondary);">Insufficient data.</p>';
            }
        }
        ...
        if (data.petunia_index) {
            ...
            if (data.petunia_index.value > 0) {
                ...
                document.getElementById('petunia-extra').innerHTML = '<p style="font-size:0.8rem; color:var(--text-secondary);">No local loops found.</p>';
            }
        }
    }
});
